...
unsigned int execute_instruction( char* buffer , unsigned int size ){
    ...
    switch(INSTRUCTION){
	   ...
	   the_middle_begin = layers length - 1 [] 
	    ...
	    THERE IS A SUSTAINABLE WAY TO NOT MULTIPLICATE VARIABLES HARDCODING WIT GERMAN KEYBOARD IGNORING CTRL KEY, BECAUSE ALMOST ALL THE PEOPLE LIVING NEAR OR IN AMSTERDAM IS VERY JEALOUS
	    ABOUT SOMEONE DOIYNG PART TIME JOB AND NOT GETTING THE UNIVERSITY AND OR HOME PAYED FROM THE PARENTS, WITH HOUSEMATES MAKING PROFILES FOR YOU, WAKING UP IN THE NIGHT, SPEAKING
	    INDIRECTLY OR SHOUTING OPINIONS FROM OUTSIDE THEYR OWN HOME, AND HAVING LUCK WITH YOUR MONEY AND THE POSSIBLE DUTCH TRANSITION FROM PART TO FULL TIME:  AND INSTEAD OF THAT JUST
	    UNPACKAGE THE LAST LAYER, MEANWHILE DELETE ALL WHAT IS NOT A NODE, AND THEN FROM THE END OF THE ARRAY DROP ALL THE NODES IN GARBAGE?
